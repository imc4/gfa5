...
namespace asgn5v1
{
	...
	public class Transformer : System.Windows.Forms.Form
	{
        ...
        // other data
        Timer timer = new Timer(); // timer for continuous rotations
        bool isInit = false; // the first time a shape is initialized (to center, scaled, etc....... not small and upside down version)
		...
		protected override void OnPaint(PaintEventArgs pea)
		{
            ...
            if (gooddata)
            {
                ...
                // checks to see if it is the first time the shape has been initialized
                if(!isInit)
                {
                    gfxInit();
                    isInit = true;
        ...
        /// <summary>
        /// Initializes the shape to the center + scale + reflect
        /// </summary>
        private void gfxInit()
        {
            double[,] net;
            // ctrans has been set to have diagonal 1s when the coordinates are loaded
            net = ctrans;
            net = Matrix.multiply(net, Matrix.translate(-vertices[0, 0], -vertices[0, 1], -vertices[0, 3]));
            net = Matrix.multiply(net, Matrix.scale(this.Height / 20 / 2, this.Height / 20 / 2, this.Height / 20 / 2));
            net = Matrix.multiply(net, Matrix.reflect(1, -1, 1));
            net = Matrix.multiply(net, Matrix.translate(this.Width / 2, this.Height / 2));
            ctrans = net;         
        }
        ...
        /// <summary>
        /// Restores shape to initial space
        /// </summary>
        ...
        /// <summary>
        /// Translate x by 75/y by 35
        /// Scale by 10%
        /// Rotate over x/y/z by .05 radians
        /// Continuous rotation over x/y/z by .05 radians
        /// Shear x with respect to y by 10%
        /// Restore initial image
        /// Close program
        /// 
        /// All transformations are done via the "net" method:
        ///     - the final transformation matrix is created by multiply individual translations matrices to a net matrix
        ///     - the actual transformation occurs during: ctrans * net
        /// 
        /// Each button calls timer.Stop() in order to make sure that the timer isn't running in the background during other transformations
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="e"></param>
		private void toolBar1_ButtonClick(object sender, System.Windows.Forms.ToolBarButtonClickEventArgs e)
		{
            // translates left by 75 units
			if (e.Button == transleftbtn)
			{
                timer.Stop();
                double[,] net = new double[4, 4];
                setIdentity(net, 4, 4);
                net = Matrix.multiply(net, Matrix.translate(-75, 0));
                ctrans = Matrix.multiply(ctrans, net);
                Refresh();
			}

            // translates right by 75 units
            ...
            // translates up by 35 units
			if (e.Button == transupbtn)
			{
                timer.Stop();
                double[,] net = new double[4, 4];
                setIdentity(net, 4, 4);
                net = Matrix.multiply(net, Matrix.translate(0, -35));
                ctrans = Matrix.multiply(ctrans, net);
                Refresh();
			}
			
            // translates down by 35 units
			...
			if (e.Button == scaleupbtn) 
			{
                ...
                double[,] net = new double[4, 4];
                setIdentity(net, 4, 4);
                net = Matrix.multiply(net, Matrix.translate(-scrnpts[0,0], -scrnpts[0,1], -scrnpts[0,2]));
                net = Matrix.multiply(net, Matrix.scale(1.1, 1.1, 1.1));
                net = Matrix.multiply(net, Matrix.translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
                ctrans = Matrix.multiply(ctrans, net);
                Refresh();
			}
			...
			if (e.Button == scaledownbtn) 
			{
                ...
                setIdentity(net, 4, 4);
                net = Matrix.multiply(net, Matrix.translate(-scrnpts[0, 0], -scrnpts[0, 1], -scrnpts[0, 2]));
                net = Matrix.multiply(net, Matrix.scale(.9, .9, .9));
                net = Matrix.multiply(net, Matrix.translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
                ctrans = Matrix.multiply(ctrans, net);
                Refresh();
			}

            // rotates over x by radians
			if (e.Button == rotxby1btn) 
			{
                ...
                setIdentity(net, 4, 4);
                net = Matrix.multiply(net, Matrix.translate(-scrnpts[0, 0], -scrnpts[0, 1], -scrnpts[0, 2]));
                net = Matrix.multiply(net, Matrix.rotate('x', .05));
                net = Matrix.multiply(net, Matrix.translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
                ctrans = Matrix.multiply(ctrans, net);
                Refresh();
			}

            // rotates over y by radians
			if (e.Button == rotyby1btn) 
			{
                ...
                setIdentity(net, 4, 4);
                net = Matrix.multiply(net, Matrix.translate(-scrnpts[0, 0], -scrnpts[0, 1], -scrnpts[0, 2]));
                net = Matrix.multiply(net, Matrix.rotate('y', .05));
                net = Matrix.multiply(net, Matrix.translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
                ctrans = Matrix.multiply(ctrans, net);
                Refresh();
			}

            // rotates over z by radians
			if (e.Button == rotzby1btn) 
			{
                ...
                setIdentity(net, 4, 4);
                net = Matrix.multiply(net, Matrix.translate(-scrnpts[0, 0], -scrnpts[0, 1], -scrnpts[0, 2]));
                net = Matrix.multiply(net, Matrix.rotate('z', .05));
                net = Matrix.multiply(net, Matrix.translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
                ctrans = Matrix.multiply(ctrans, net);
                Refresh();
			}

            // continous rotation over x using timers
			if (e.Button == rotxbtn) 
			{
                timer.Stop();         
                runTimer(1, 'x', .05);
            ...
            // continuous rotation over y using timers
			if (e.Button == rotybtn) 
			{
                timer.Stop();      
                runTimer(1, 'y', .05);
            ...
            // continuous rotation over z using timers
			if (e.Button == rotzbtn) 
			{
                timer.Stop(); 
                runTimer(1, 'z', .05);
			...
			if(e.Button == shearleftbtn)
			{
                ...
                setIdentity(net, 4, 4);
                double y = ctrans[3, 1];
                net = Matrix.multiply(net, Matrix.translate(0, -y, 0));
                net = Matrix.multiply(net, Matrix.shear(.1));
                net = Matrix.multiply(net, Matrix.translate(0, y, 0));
                ctrans = Matrix.multiply(ctrans, net);
                Refresh();
			}

            // shears to the right by 10%
        ...
        /// <summary>
        /// sets timer, continuously rotates over specified axis
        /// </summary>
        /// <param name="axis">the axis to rotate over</param>
        /// <param name="rad">the number of radians per rotation</param>
        public void runTimer(int interval, char axis, double rad)
        {
            timer = new Timer();
            ...
            timer.Tick += new EventHandler((sender, e) =>
            {
                ...
                setIdentity(net, 4, 4);
                net = Matrix.multiply(net, Matrix.translate(-scrnpts[0, 0], -scrnpts[0, 1], -scrnpts[0, 2]));
                net = Matrix.multiply(net, Matrix.rotate(axis, rad));
                net = Matrix.multiply(net, Matrix.translate(scrnpts[0, 0], scrnpts[0, 1], scrnpts[0, 2]));
                ctrans = Matrix.multiply(ctrans, net);
                Refresh();     
            });
